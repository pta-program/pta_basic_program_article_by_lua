...
每次比较都会将查找区间缩小约一半，因此时间复杂度为 `O(log n)`；函数只维护左右边界和中间位置，额外空间复杂度为 `O(1)`。

## 常见易错点

1. 二分查找的前提是 `T.R` 按关键字严格递增，不能用于无序表。
2. Lua 下标从 `1` 开始，初始区间应为 `[1, T.length]`。
3. 查找失败时必须返回 `0`，裁判程序再将其转换为 `NOT FOUND`。
4. 找到目标后应立即返回 `mid`；更新区间时使用 `mid - 1` 或 `mid + 1`，避免区间不收缩造成死循环。
