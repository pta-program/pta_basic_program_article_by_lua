...
函数只遍历一次系数数组，时间复杂度为 `O(n)`；只使用 `sum` 和 `power` 等固定数量的变量，额外空间复杂度为 `O(1)`。

## 常见易错点

1. Lua 数组从1开始，`a[1]`对应常数项，不能误当作 `x` 的一次项。
2. 每轮应先更新 `power = power * x`，再处理当前的 `a[i]`，这样 `a[2]`对应 `x^1`。
3. `n = 0` 时没有进入循环，直接返回 `a[1]` 即可。
