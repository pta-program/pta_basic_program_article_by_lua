...
函数从第二个元素开始线性比较一次，时间复杂度为 `O(n)`；除输入数组外只保存当前最大值，额外空间复杂度为 `O(1)`。

## 常见易错点

1. 应将 `list[1]` 作为初始最大值，不能把最大值初始化为 `0`，否则全为负数时会出错。
2. 第一个元素已经参与初始化，比较循环应从 `2` 开始。
3. 题目中的 `n` 表示元素个数，必须保证数组非空后才能访问 `list[1]`。
