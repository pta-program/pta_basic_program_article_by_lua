...
设链表有 `L` 个结点，第 `i` 个结点的数据为 `vᵢ`。遍历和阶乘计算的总时间复杂度为 `O(L + ∑vᵢ)`，额外空间复杂度为 `O(1)`。

## 常见易错点

1. 遍历结点后必须执行 `p = p.Next`，否则会重复处理同一结点并造成死循环。
2. 阶乘累积值应从 `1` 开始，`0!` 的值也是 `1`。
3. 空链表应直接返回 `0`；不要在空链表上访问 `head.Data`。
4. 主程序使用头插法建立链表，但阶乘和与结点访问顺序无关。
