...
设 `n` 的十进制位数为 `k`。函数逐位检查一次，时间复杂度为 `O(k)`，只使用计数器和当前待处理数，空间复杂度为 `O(1)`。

## 常见易错点

1. 统计前要对 `n` 取绝对值，负号不是数字，不能参与计数。
2. `n == 0` 时循环不会执行，但数字 `0` 本身应在 `d == 0` 时计数一次。
3. `n // 10` 用于去掉最低位，不能忘记更新 `n`，否则会陷入死循环。
4. 不应把高位补出的前导零计入次数，题目统计的是整数实际表示中的数字。
