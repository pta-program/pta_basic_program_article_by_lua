...
设循环共计算 k 个级数项，时间复杂度为 O(k)，额外空间复杂度为 O(1).

## 常见易错点

- 终止判断应针对刚计算出的新项，且该项仍需先加入总和。
- 输出的是 2 倍的级数和，并且必须保留 6 位小数。

## 更多测试

边界测试：输入 0.5，第一项 1/3 小于阈值，预期输出 2.666667。

特殊测试：输入 0.001，验证更小阈值会继续累加更多项。
