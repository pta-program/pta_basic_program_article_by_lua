...
设分子为 p、分母为 q，辗转相除法时间复杂度为 O(log(min(p,q)))，额外空间复杂度为 O(1).

## 常见易错点

- 应使用普通字符串匹配解析分子和分母，约分时只对整数使用最大公约数。
- 分子为 0 时结果应化为 0/1，不能保留原分母。

## 更多测试

边界测试：输入 48/18，预期输出 8/3。

特殊测试：输入 0/12，预期输出 0/1，验证分子为零时仍能约分。
