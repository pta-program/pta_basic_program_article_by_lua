...
程序只解析两个数并进行一次时间换算，时间复杂度为 O(1)，额外空间复杂度为 O(1).

## 常见易错点

- 起始时间应拆成小时和分钟后统一换算为总分钟，不能直接对 HHMM 数值相加。
- 输出分钟必须补足两位，并正确处理跨小时和负时间偏移。

## 更多测试

边界测试：输入 2359 2，预期输出 2401，验证跨小时处理。

特殊测试：输入 530 -40，预期输出 450，验证负的时间偏移。
