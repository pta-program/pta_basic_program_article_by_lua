...
设有 n 个有理数，每次通分和约分都使用辗转相除法；忽略大整数算术成本时，时间复杂度约为 O(n log M)，额外空间复杂度为 O(1).

## 常见易错点

- 平均值应先完成所有有理数求和，再把总和的分母乘以 N。
- 每次通分后及时约分可避免数值过大，最终结果仍需化为最简分数。

## 更多测试

边界测试：输入 1 个数 1/2，预期输出 1/2。

特殊测试：输入 2 个数 1/2 -1/2，预期输出 0，验证均值为零。
