...
程序只计算加、减、乘、除四种复数运算并格式化固定数量的结果，时间复杂度为 O(1)，额外空间复杂度为 O(1).

## 常见易错点

- 复数除法的分母是 a2²+b2²，除数为 0+0i 时不能执行除法。
- 接近 0 的实部或虚部要按 0.05 阈值处理，并严格控制正负号和小数位。

## 更多测试

边界测试：除数复数为 1+0i 时，验证除法结果等于被除数。

特殊测试：结果实部或虚部绝对值小于 0.05 时，验证按题目规则输出 0.0 或纯虚/纯实形式。
