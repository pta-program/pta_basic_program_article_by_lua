...
设 N 的整数分拆数量为 p(N)，程序先后进行两次 DFS，时间复杂度为 O(p(N)·N)，递归路径和当前方案存储空间复杂度为 O(N).

## 常见易错点

- 递归下一层的起点应为当前项，保证分解序列非递减并避免重复方案。
- 分号、每行 4 个式子以及最后一个式子的换行位置必须单独处理。

## 更多测试

边界测试：输入 1，预期输出 1=1。

特殊测试：输入 7，验证每 4 个分解式换行且最后一个式子后换行。
