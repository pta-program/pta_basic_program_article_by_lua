...
程序只读取 4 个标记并进行一次格式化输出，时间复杂度为 O(1)，额外空间复杂度为 O(1).

## 常见易错点

- 四个输入项的读取顺序与输出顺序不同，字符不能误当成数值处理。
- 两个浮点数都必须使用两位小数输出，整数项不能套用浮点格式。

## 更多测试

边界测试：输入 0 0 a 0，预期输出 a 0 0.00 0.00。

特殊测试：输入 -3.5 -8 z 12.345，预期输出 z -8 -3.50 12.35。
